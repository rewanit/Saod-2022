...
public class RB
{
    ...
    {
		var node = root;
		int max =  AbsolutliNotRecursionMethod(root);
		return max-1;

    }

    private int AbsolutliNotRecursionMethod(Node root,int current =0)
    {
        if (root == null)
        {
			return current;
		...
		current +=1;
		var x = AbsolutliNotRecursionMethod(root.left,current);
		var y = AbsolutliNotRecursionMethod(root.right, current);
		return x > y ? x : y;
